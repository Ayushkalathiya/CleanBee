...
export default function Header({ onMenuClick, totalEarnings }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    fetchBalance();
    //set balance
    const handleBalance = (event: CustomEvent) => {
      if (event.detail) {
        setBalance(event.detail);
      }
    };

    window.addEventListener("balance", handleBalance as EventListener);

    return () => {
        window.removeEventListener("balance", handleBalance as EventListener);
    }
  }, [userInfo]);
